...
class DataExporter:
    ...
    def template_aware_algorithm(self, input_df: pd.DataFrame, groupby_cols: list[str]) -> pd.DataFrame:
        ...
        # Aggregate the data
        agg_df = grouped_df.agg(agg_dict).rename(columns={"num_issues": "num_pages_impacted"})
        ...
        # Sort the data
        agg_df = agg_df.sort_values(by="num_pages_impacted", ascending=False)
    ...
    def run_axe_core_audit_template_aware(self) -> pd.DataFrame:
        ...
        for i, _ in enumerate(original_column_order):
            if original_column_order[i] == "num_issues":
                original_column_order[i] = "num_pages_impacted"
    ...
    def generate_axe_core_template_aware_query(
        ...
        query = """
             SELECT organisation, base_url, COUNT(*) as num_count
                    FROM cwac_table
                    WHERE num_pages_impacted > 0
                    AND "best-practice" = 'No'
                    GROUP BY base_url
                    UNION
                    SELECT organisation, base_url, num_pages_impacted
                    FROM cwac_table
                    WHERE num_pages_impacted = 0
                    AND "best-practice" = 'No'
                    AND base_url NOT IN (
